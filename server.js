...
function createComment(url, request) {
  ...
  //If body is supplied, user with supplied username, and article with supplied article ID exists then add to database
  if (requestComment && requestComment.username && database.articles[requestComment.articleId] &&
    ...
    // Creatte the comment object using the information from the create/post request
    const comment = {
    ...
    database.comments[comment.id] = comment; // we add the new comment object we just created to the database
    database.users[comment.username].commentIds.push(comment.id); // update user info list in the database to indicate that this particular user created a comment
    database.articles[comment.articleId].commentIds.push(comment.id); 

    response.body = {comment: comment}; // setup the body of APIs response to hold the comment just created
    response.status = 201; 
  } else {
    response.status = 400; //unsuccessful at creating the new comment
  }
...
function updateComment(url, request) {
  ...
  if (!id || !requestComment) {
    response.status = 400; //no ID or updated comment is supplied
  } else if (!savedComment) {
    response.status = 404; //comment with given id does not exist
  } else {
    //update body of corresponding comment in database, and return 200 response with the updated comment on comment property of response body
...
function deleteComment(url,request) {
  ...
  if (savedComment) {
    ...
    response.status = 404; //If no ID is supplied or comment with supplied ID doesn’t exist, returns 404 respons
  }
  return response;
}
...
function upvoteComment(url, request) {
  ...
  if ( savedComment && database.users[username]) {
    //Adds supplied username to upvotedBy of corresponding comment if user hasn’t already upvoted the comment, removes username 
    //from downvotedBy if that user had previously downvoted the comment, returns 200 response with comment on comment property of response body
    savedComment = upvote(savedComment, username);

    response.body = {comment: savedComment};
    response.status = 200; //successful upvote
  }
  else{
...
function downvoteComment(url, request) {
  ...
  if (savedComment && database.users[username]) {
    //Adds supplied username to downvotedBy of corresponding comment if user hasn’t already downvoted the comment, remove username from upvotedBy 
    //if that user had previously upvoted the comment, returns 200 response with comment on comment property of response body
    savedComment = downvote(savedComment, username);

    response.body = {comment: savedComment};
    response.status = 200; //sucessful downvote
  } else {
    //If no ID is supplied, comment with supplied ID doesn’t exist, or user with supplied username doesn’t exist, returns 400 response
    response.status = 400;
  }
